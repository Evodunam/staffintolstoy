...
function toast({ duration, ...props }: Toast & { duration?: number }) {
  const id = genId()
  const autoDismissDuration = duration ?? TOAST_REMOVE_DELAY
  ...
  // Defer ADD_TOAST to next tick to avoid "Maximum update depth exceeded" when toast is called from React Query onSuccess or during commit (Radix Toast setRef can trigger state updates).
  const payload = {
    ...props,
    id,
    open: true,
