  ...
  return (
    <SortableContext items={columnIds} strategy={rectSortingStrategy}>
      <div
        ...
        className={cn("grid auto-rows-fr gap-6 grid-cols-1 md:grid-cols-[repeat(4,minmax(280px,1fr))]", className)}
      >
        {children}
      </div>
    </SortableContext>
...
function KanbanColumn({ value, className, children, disabled }: KanbanColumnProps) {
  ...
  const style = {
    ...
    minWidth: 0, // allow column to shrink within grid cell and prevent overflow/overlap
  ...
  return (
    <ColumnContext.Provider value={{ attributes, listeners, isDragging: isColumnDragging, disabled }}>
      <div
        ...
        style={style}
        className={cn(
          "group/kanban-column flex w-full min-w-0 flex-col rounded-xl border border-border bg-muted/30 p-4",
...
function KanbanColumnContent({ value, className, children }: KanbanColumnContentProps) {
  ...
  return (
    <SortableContext items={itemIds} strategy={verticalListSortingStrategy}>
      <div data-slot="kanban-column-content" className={cn("flex flex-col gap-3 flex-1 overflow-y-auto max-h-[60vh] min-h-[120px]", className)}>
        {children}
      </div>
    </SortableContext>
  );
