...
async function getCompaniesNeedingReplenishment(): Promise<CompanyCommitments[]> {
    ...
    .where(
      and(
        ...
        or(
          ...
          and(
            isNotNull(profiles.mtCounterpartyId),
            isNotNull(profiles.mtExternalAccountId)
          )
        )
      )
    );
  ...
  for (const company of companies) {
    ...
    const hasBankAccount = company.mtCounterpartyId && company.mtExternalAccountId;
...
async function processAutoReplenishment(company: CompanyCommitments): Promise<boolean> {
  try {
    ...
    const hasBankAccount = profile.mtCounterpartyId && profile.mtExternalAccountId;
    ...
    if (hasBankAccount) {
      try {
        ...
        console.log(`[AutoReplenish] Created ACH payment order ${paymentOrder.id} for $${(company.shortfallCents / 100).toFixed(2)}`);
        ...
        // Update balance (ACH may be pending but we credit immediately for sandbox)
        ...
        console.log(`[AutoReplenish] ACH debit failed: ${achError.message}, trying card...`);
