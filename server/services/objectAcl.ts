import { S3Client, HeadObjectCommand, CopyObjectCommand, GetObjectCommand } from "@aws-sdk/client-s3";
import { S3Object, getS3Client } from "./objectStorage";
...
export interface ObjectAccessGroup {
  ...
  //   group db entity could contain a bunch of user ids. User needs to be a
  //   member of the group to be able to access the object.
  // - for SUBSCRIBER, the id could be the subscriber db entity id, and the
  //   subscriber db entity could contain a bunch of user ids. User needs to be
  //   a subscriber to be able to access the object.
  id: string;
}
...
// Note: S3 requires copying the object to update metadata. This is expensive but necessary.
...
export async function getObjectAclPolicy(
  ...
  // Create bucket-specific client for IDrive E2 subdomain buckets
  ...
  const bucketEndpoint = `${objectFile.bucket}.${baseEndpoint}`;
  ...
  try {
    ...
    const metadata = await bucketS3Client.send(command);
